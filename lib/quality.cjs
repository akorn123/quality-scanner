const {
  ...
  const evaluateQualityGate = ({
    ...
    const structure = quality.structure ?? {
      fatal: 0,
      errors: 0,
      score: 100,
    };
    ...
    if (
      ...
      (structure.fatal ||
        structure.errors ||
        (thresholds.structure != null &&
          scoreFails(structure.score, thresholds.structure)))
    ) {
      selectedFailures.push('structure');
    }
  ...
  const getReleaseConfidence = ({
    ...
    if (structure.fatal > 0 || structure.errors > 0) {
      checks.push({
        id: 'structure-errors',
        level: 'blocked',
        concern: 'structure',
        severities: ['fatal', 'error'],
        message: `${structure.fatal + structure.errors} structure error finding(s).`,
      });
    }
    ...
    const overallScore =
      weighted([
        ...
        },
        {
          ...
          weight: config.weights.structure ?? 0,
        },
      ]);
