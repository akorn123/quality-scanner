...
const createFinding = ({ id, category, severity, penalty, line, code, description, suggestion, endpoint }) => ({
  id,
  concern: 'security',
  category,
  severity,
  ...
  for (const file of files) {
    ...
    for (const call of calls) {
      ...
      for (const arg of middlewareArgs) {
        const identifierMatch = arg.trim().match(/^([A-Za-z_$][\w$]*)$/);
        if (!identifierMatch) continue;
        const targetFile = imports.get(identifierMatch[1]);
...
const analyzeFile = (file, config, mounts) => {
  ...
  for (const call of routeCalls) {
    ...
    const externallyProtected = externalMounts.some((mount) => mount.protected);
    const externallyAuthorized = externalMounts.some((mount) => mount.authorized);
    const externallyRateLimited = externalMounts.some((mount) => mount.rateLimited);
  ...
  return {
    ...
    framework: routeCalls.length || uses.length ? 'express' : null,
    endpoints,
    findings,
    score,
  };
