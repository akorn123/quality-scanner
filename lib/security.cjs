...
const addSecurityFinding = ({
  ...
  if (!suppression) {
    findings.push(explainInvalidSuppression(lines, call.line - 1, rule, finding));
    return;
  }
...
const analyzeSecurity = ({ files, config }) => {
  ...
  const analyzed = files.map((file) =>
    analyzeFile(
      file,
      securityConfig,
      mountMap.get(resolve(file)) ?? [],
    ),
  );

  const reportable = analyzed
    .filter((result) => result.framework || result.findings.length || result.endpoints.length);

  if (
    reportable.length ||
    analyzed.length === 0 ||
    securityConfig.frontendEnabled === false
  ) {
    return reportable;
  }

  /*
   * Frontend rules scan every supplied source file. Preserve one clean
   * result so a completed scan with no findings scores 100 instead of N/A,
   * without diluting findings by averaging every clean file into the score.
   */
  return [
    {
      ...analyzed[0],
