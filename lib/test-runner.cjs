  ...
  const runProcess = ({
    ...
    new Promise((resolve, reject) => {
      ...
      const onChunk = (chunk, target) => {
        ...
        const planMatch = text.match(TAP_PLAN);
        if (planMatch) {
          /*
           * TAP plan can refine the expected total. createDeterminateProgress
           * is constructed with progressTotal (test-file count); keeping that
           * total is fine when a better dynamic total is unavailable.
           */
          void planMatch;
        }

        const matches = text.match(RESULT_TOKEN);
        if (matches?.length) {
          matchCount += matches.length;
          progress.update(matchCount);
        }
      };
