const {
  ...
  const { isFreshToday } = require('./coverage.cjs');
  ...
  const runProjectTests = async ({
    ...
    if (reuseArtifacts) {
      ...
      const resultsOk =
        ...
        isFreshToday(paths.resultsFile, config);

      const coverageOk =
        existsSync(paths.coverageSummary) &&
        isFreshToday(paths.coverageSummary, config);

      if (resultsOk && coverageOk) {
        if (!ciMode) {
      ...
      if (!ciMode) {
        console.log(
          'Existing artifacts missing or stale; regenerating.',
        );
      }
    }  
    if (!ciMode) {
