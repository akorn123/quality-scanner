...
const parseDashboardPort = (value) => {
  ...
  if (!Number.isInteger(port) || port < 0 || port > 65535) {
    throw new Error(
      'Dashboard port must be an integer from 0 to 65535 (0 selects an available port).',
    );
  }
...
const startReportServer = (config) =>
  new Promise((resolve, reject) => {
    const requestedPort = parseDashboardPort(config.dashboard?.port ?? 0);
    ...
    server.listen(requestedPort, '127.0.0.1', () => {
      ...
      if (!port) {
        reject(
          new Error(
            'Quality report server failed to bind a port.',
          ),
        );
        return;
      }
