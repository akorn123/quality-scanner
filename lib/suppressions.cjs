...
const explainInvalidSuppression = (lines, zeroBasedLineIndex, rule, finding) => {
  const previousLine = lines[zeroBasedLineIndex - 1] ?? '';
  const directive = previousLine.match(
    /^\s*\/\/\s*quality-scanner-ignore-next-line\s+(.+?)\s*$/,
  );
  if (!directive) return finding;

  const [targetText, ...reasonParts] = directive[1].split(/\s+--(?:\s+|$)/);
  const targets = parseTargets(targetText);
  if (
    reasonParts.join(' -- ').trim() ||
    !targets.some((target) => targetMatchesRule(target, rule))
  ) {
    return finding;
  }

  const explanation =
    `The quality-scanner-ignore-next-line comment on line ${zeroBasedLineIndex} did not suppress this finding because it is missing a reason. ` +
    `Add a non-empty reason after " -- ", for example: // quality-scanner-ignore-next-line ${targets.join(',')} -- Explain why this exception is justified.`;
  return {
    ...finding,
    description: [finding.description, explanation].filter(Boolean).join(' '),
  };
};
