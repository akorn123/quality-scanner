...
const rel = (file) => toRelativePath(file);
...
const resolveLcovReportDir = (config) => {
  const existing = (config.lcovReportDirs ?? [])
    .map((p) => join(process.cwd(), p))
    .filter(existsSync);

  if (!existing.length) return null;

  const normalized = existing.map((dir) => ({
    dir,
    key: normalize(dir).replaceAll('\\', '/'),
  }));

  const preferred = normalized.filter((item) => /\/lcov-report$/i.test(item.key));
  const pool = preferred.length ? preferred : normalized;

  return pool.sort((a, b) => b.key.length - a.key.length)[0].dir;
};

const toArtifactSourceKey = (reportDir, file) => {
  let key = normalize(relative(reportDir, file))
    .replaceAll('\\', '/')
    .replace(/\.html$/, '');

  key = key.replace(/^lcov-report\//, '');
  return key;
};
...
const scanCoverageHtml = (file, reportDir) => {
  ...
  const sourceFile = toArtifactSourceKey(reportDir, file);
...
const analyzeCoverageArtifacts = (testabilityResults, config) => {
  const reportDir = resolveLcovReportDir(config);
  if (!reportDir) {
    return {
      source: null,
      eMarkers: 0,
  ...
  const byFile = artifacts.map((artifact) => {
    ...
    const lines = artifact.lines.map((line) =>
      classifyMarkerLine({
        artifactLine: line,
        result,
        exclusionRules: config.coverageExclusionRules,
      }),
    );
    return {
      ...artifact,
