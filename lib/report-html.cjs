...
const metricCard = ({
  ...
  if (!href) {
    return `
      <div class="metric-card"${idAttribute}>
        ${content}
      </div>
    `;
  }
...
const releaseCheckTarget = (
  check,
  { blockingDetails = true } = {},
) => {
  if (blockingDetails && check.level === 'blocked' && check.id) {
    return `#release-check-${check.id}`;
  }

  if (check.id === 'unclassified-branches') {
    return '#coverage-accountability';
  }

  const targets = {
    tests: '#test-pass-rate',
    behavior: '#behavior-findings',
    testability: '#testability-findings',
  ...
  return targets[check.concern] ?? '#raw-reports';
};
...
const renderBlockingRelease = ({
  ...
  return blockingChecks
    .map((check) => {
      const target = releaseCheckTarget(check, {
      ...
      const findingHtml = findings.length
        ...
        : `
          <a
            class="blocking-summary-note blocking-summary-link"
            href="${esc(target)}"
          >
            ${esc(check.message)}
          </a>
      ...
      return `
        ...
        >
          <a
            class="blocking-check-header blocking-check-header-link"
            href="${esc(target)}"
            title="View the related ${esc(check.concern)} details"
          >
            ...
            <span class="release-check-arrow" aria-hidden="true">→</span>
          </a>
...
const renderReleaseChecks = (quality) => {
  ...
  return quality.releaseChecks
    .map((check) => {
      ...
      return `
        <a
          class="release-check release-check-link"
          href="${esc(target)}"
          title="View the related ${esc(check.concern)} details"
        >
...
const renderHtml = ({
  ...
  const structureFindings =
    structure.summary.totalFindings ??
    structure.summary.findings ??
    0;
...
.blocking-check-header-link:hover {
  background: #fee2e2;
}
...
.release-check-link:focus-visible,
.blocking-check-header-link:focus-visible,
.blocking-summary-link:focus-visible,
.blocking-finding:focus-visible {
  outline: 3px solid rgba(37, 99, 235, 0.35);
  outline-offset: 2px;
}

.metric-card:target,
.blocking-check:target,
section:target {
  border-color: var(--primary);
...
:root[data-theme="dark"] .report-nav a,
:root[data-theme="dark"] .blocking-description,
:root[data-theme="dark"] th {
  color: #cbd5e1;
}

:root[data-theme="dark"] .nav-count,
:root[data-theme="dark"] .chip.neutral {
  color: #cbd5e1;
  background: #334155;
}
...
:root[data-theme="dark"] .chip.error {
  color: #fecaca;
  background: #7f1d1d;
}
...
:root[data-theme="dark"] .score-track {
  background: #334155;
}

:root[data-theme="dark"] .small-card,
:root[data-theme="dark"] th {
  background: #0f172a;
}

:root[data-theme="dark"] .blocking-check,
:root[data-theme="dark"] .source-card {
  background: var(--surface);
}

:root[data-theme="dark"] .blocking-check,
:root[data-theme="dark"] .blocking-check-header {
  border-color: #7f1d1d;
}

:root[data-theme="dark"] .blocking-check-header-link:hover {
  background: #3f1218;
}
...
:root[data-theme="dark"] .finding-line {
  background: #3b2415;
}
...
:root[data-theme="dark"] .report-nav a:hover {
  color: var(--primary);
  background: var(--primary-soft);
}
...
<div class="report-shell">
  ...
  >
    ...
    ${renderFindingsSection(
      ...
      } with recognized protection. Browser security checks are included for frontend source files.`,
    )}

    ${renderFindingsSection(
      'Structure Findings',
      structure,
      `Path and organization rules, including naming, required/forbidden paths, test placement, and shared symbol placement. Categories: ${Object.entries(
        structure.summary.categories ?? {},
      ).map(([category, count]) => `${category} (${count})`).join(', ') || 'none'}.`,
    )}
...
(() => {
  ...
  const applyTheme = (theme) => {
    const selected = isTheme(theme) ? theme : 'dark';
  ...
  const readPreference = async () => {
    try {
      ...
      if (isTheme(preferences.theme)) applyTheme(preferences.theme);
    } catch {
      // The embedded preference remains active for a file:// report.
    }
  };
  ...
  toggle.addEventListener('click', async () => {
    ...
    try {
      ...
      applyTheme(previous);
      toggle.title = 'Theme could not be saved; run the dashboard server and try again.';
    } finally {
